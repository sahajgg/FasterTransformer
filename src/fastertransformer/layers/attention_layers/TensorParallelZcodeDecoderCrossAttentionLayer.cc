...
template<typename T>
TensorParallelZcodeDecoderCrossAttentionLayer<T>::TensorParallelZcodeDecoderCrossAttentionLayer(
...
TensorParallelZcodeDecoderCrossAttentionLayer<T>::TensorParallelZcodeDecoderCrossAttentionLayer(
    ...
    std::shared_ptr<AbstractCustomComm> custom_all_reduce_comm,
    int                                 enable_custom_all_reduce):
    TensorParallelZcodeDecoderCrossAttentionLayer(max_batch_size,
                                             head_num,
                                             size_per_head,
...
template<typename T>
TensorParallelZcodeDecoderCrossAttentionLayer<T>::TensorParallelZcodeDecoderCrossAttentionLayer(
    TensorParallelZcodeDecoderCrossAttentionLayer<T> const& attention_layer):
    ZcodeDecoderCrossAttentionLayer<T>(attention_layer),
...
template<typename T>
void TensorParallelZcodeDecoderCrossAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                                          ...
                                                          const AttentionWeight<T>* attention_weights)
{
    ...
    ZcodeDecoderCrossAttentionLayer<T>::forward(output_tensors, input_tensors, attention_weights);

    T* attention_out = output_tensors->getPtr<T>("hidden_features");
    if (tensor_para_.world_size_ > 1) {
        if (!use_custom_all_reduce_kernel) {
            ftNcclAllReduceSum(
                attention_out, attention_out, size, tensor_para_, ZcodeDecoderCrossAttentionLayer<T>::stream_);
        }
        else {
            custom_all_reduce_comm_->customAllReduce(size, ZcodeDecoderCrossAttentionLayer<T>::stream_);
            output_tensors->at("hidden_features").data = reduce_tensor[0].data;
        }
...
template class TensorParallelZcodeDecoderCrossAttentionLayer<float>;
template class TensorParallelZcodeDecoderCrossAttentionLayer<half>;
#ifdef ENABLE_BF16
template class TensorParallelZcodeDecoderCrossAttentionLayer<__nv_bfloat16>;
#endif
