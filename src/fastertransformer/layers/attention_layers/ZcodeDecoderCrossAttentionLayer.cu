...
template<typename T>
__global__ void zcode_cross_attention_kernel(T*          query_buf,
                                       const T*    Q_bias,
                                       T*          key_cache,
                                       const T*    K_bias,
                                       T*          value_cache,
...
__global__ void zcode_cross_attention_kernel_opt(T* __restrict query_buf,
...
template<typename T>
void zcode_cross_attention_dispatch(T*                               query_buf,
                              const T*                         Q_bias,
                              T*                               key_cache,
                              const T*                         K_bias,
                              T*                               value_cache,
                              ...
                              cudaStream_t                     stream)
{
    if (!batch_major_cache) {
        ...
        float     scalar   = 1.f / sqrtf(size_per_head * 1.0 * q_scaling);
        ...
        switch (cond) {
            case 32:
                zcode_cross_attention_kernel_opt<T, 32, block_sz>
                ...
                break;
            case 64:
                zcode_cross_attention_kernel_opt<T, 64, block_sz>
                    <<<grid, block_sz, sizeof(float) * memory_max_len, stream>>>(query_buf,
                                                                                 Q_bias,
                                                                                 key_cache,
                                                                                 K_bias,
                ...
                break;
            case 128:
                zcode_cross_attention_kernel_opt<T, 128, block_sz>
                    <<<grid, block_sz, sizeof(float) * memory_max_len, stream>>>(query_buf,
                                                                                 Q_bias,
                                                                                 key_cache,
                                                                                 K_bias,
            ...
            default:
                ...
                zcode_cross_attention_kernel<T><<<grid, block, shared_size, stream>>>(query_buf,
                                                                                Q_bias,
                                                                                key_cache,
                                                                                K_bias,
                                                                                value_cache,
...
template void zcode_cross_attention_dispatch(float*                           query_buf,
                                       const float*                     Q_bias,
                                       float*                           key_cache,
                                       const float*                     K_bias,
                                       float*                           value_cache,
...
template void zcode_cross_attention_dispatch(half*                            query_buf,
                                       const half*                      Q_bias,
                                       half*                            key_cache,
                                       const half*                      K_bias,
                                       half*                            value_cache,
...
#ifdef ENABLE_BF16
template void zcode_cross_attention_dispatch(__nv_bfloat16*                   query_buf,
...
template<typename T>
__global__ void zcode_transpose_4d_batch_major_mem_k_cache(
...
template<typename T>
__global__ void zcode_transpose_4d_batch_major_mem_v_cache(
...
template<typename T>
void zcode_transpose_4d_batch_major_memory_kernelLauncher(T*           dst,
                                                    const T*     src,
                                                    ...
                                                    cudaStream_t stream)
{
    ...
    if (k_cache) {
        zcode_transpose_4d_batch_major_mem_k_cache<<<grid, block_sz, 0, stream>>>(
            dst, src, local_head_num, size_per_head, max_seq_len);
    }
    else {
        zcode_transpose_4d_batch_major_mem_v_cache<<<grid, block_sz, 0, stream>>>(
            dst, src, local_head_num, size_per_head, max_seq_len);
    }
}

template void zcode_transpose_4d_batch_major_memory_kernelLauncher(float*       dst,
                                                             const float* src,
                                                             const int    local_batch_size,
                                                             const int    max_seq_len,
                                                             const int    size_per_head,
...
template void zcode_transpose_4d_batch_major_memory_kernelLauncher(half*        dst,
                                                             const half*  src,
                                                             const int    local_batch_size,
                                                             const int    max_seq_len,
                                                             const int    size_per_head,
...
#ifdef ENABLE_BF16
template void zcode_transpose_4d_batch_major_memory_kernelLauncher(__nv_bfloat16*       dst,
...
template<typename T>
void ZcodeDecoderCrossAttentionLayer<T>::allocateBuffer()
{
    FT_CHECK(false);
}
...
void ZcodeDecoderCrossAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t max_mem_seq_len, size_t seq_len)
{
    ...
    if(seq_len == 1){
        ...
        q_buf_step0_ = (T*)(allocator_->reMalloc(q_buf_step0_, sizeof(T) * batch_size * seq_len * hidden_units_, false));
        q_buf_2_step0_ = (T*)(allocator_->reMalloc(q_buf_2_step0_, sizeof(T) * batch_size * seq_len * hidden_units_, false));
        qk_buf_step0_ = (T*)(allocator_->reMalloc(qk_buf_step0_, sizeof(T) * batch_size * head_num_ * seq_len * max_mem_seq_len, false));
        mem_cache_buf_2_step0_ = (T*)(allocator_->reMalloc(mem_cache_buf_2_step0_, sizeof(T) * batch_size * max_mem_seq_len * hidden_units_, false));
        context_buf_step0_ = (T*)(allocator_->reMalloc(context_buf_step0_, sizeof(T) * batch_size * seq_len * hidden_units_, false));
        context_buf_2_step0_ = (T*)(allocator_->reMalloc(context_buf_2_step0_, sizeof(T) * batch_size * seq_len * hidden_units_, false));
        attention_mask_step0_ = (T*)(allocator_->reMalloc(attention_mask_step0_, sizeof(T) * batch_size * max_mem_seq_len * max_mem_seq_len, false));
    }

    if (is_batch_major_cache_) {
        mem_cache_buf_ = (T*)(allocator_->reMalloc(mem_cache_buf_, sizeof(T) * batch_size * max_mem_seq_len * hidden_units_, false));
    }
    is_allocate_buffer_ = true;
}

template<typename T>
void ZcodeDecoderCrossAttentionLayer<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        if (q_buf_step0_ != nullptr){
            allocator_->free((void**)(&q_buf_step0_));
            allocator_->free((void**)(&q_buf_2_step0_));
            allocator_->free((void**)(&qk_buf_step0_));
            allocator_->free((void**)(&context_buf_step0_));
            allocator_->free((void**)(&context_buf_2_step0_));
            allocator_->free((void**)(&attention_mask_step0_));
            allocator_->free((void**)(&mem_cache_buf_2_step0_));
        }
        else{
...
template<typename T>
ZcodeDecoderCrossAttentionLayer<T>::ZcodeDecoderCrossAttentionLayer(size_t           max_batch_size,
                                                          size_t           head_num,
...
ZcodeDecoderCrossAttentionLayer<T>::ZcodeDecoderCrossAttentionLayer(size_t           max_batch_size,
                                                          ...
                                                          IAllocator*      allocator,
                                                          bool             is_free_buffer_after_forward):
    ZcodeDecoderCrossAttentionLayer<T>(max_batch_size,
                                  head_num,
                                  size_per_head,
...
template<typename T>
ZcodeDecoderCrossAttentionLayer<T>::ZcodeDecoderCrossAttentionLayer(ZcodeDecoderCrossAttentionLayer<T> const& attention_layer):
    ZcodeDecoderCrossAttentionLayer(attention_layer.max_batch_size_,
...
template<typename T>
ZcodeDecoderCrossAttentionLayer<T>::~ZcodeDecoderCrossAttentionLayer()
{
...
template<typename T>
void ZcodeDecoderCrossAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                            TensorMap*                input_tensors,
                                            const AttentionWeight<T>* attention_weights)
{
    // input tensors:
    //      attention_input [batch_size, d_model] or [batch_size, seq_len, d_model] for step=0,
    ...
    if(step == 0){
        ...
        cublas_wrapper_->stridedBatchedGemm(CUBLAS_OP_T,
                                        ...
                                        mem_cache_buf_2_step0_,
                                        size_per_head_,
                                        mem_max_seq_len * size_per_head_,
                                        q_buf_2_step0_,
                                        size_per_head_,
                                        seq_len * size_per_head_,
                                        qk_buf_step0_,
                                        mem_max_seq_len,
                                        seq_len * mem_max_seq_len,
                                        batch_size * head_num_, /* batch size */
                                        1.0f /* alpha */);
        sync_check_cuda_error();

        // softmax(QK)
        MaskedSoftmaxParam<T, T> param;
        param.attention_score    = qk_buf_step0_;         // (batch_size, head_num, q_length, k_length)
        param.qk                 = qk_buf_step0_;         // (batch_size, head_num, q_length, k_length)
        param.attention_mask     = attention_mask_step0_;  // (batch_size, q_length, k_length)
        param.batch_size         = batch_size;
        param.q_length           = seq_len;
        param.k_length           = mem_max_seq_len;
        ...
        invokeTransposeQKV(context_buf_2_step0_,
                           context_buf_step0_,
                           batch_size,
                           seq_len,
                           head_num_,
...
template class ZcodeDecoderCrossAttentionLayer<float>;
template class ZcodeDecoderCrossAttentionLayer<half>;
#ifdef ENABLE_BF16
template class ZcodeDecoderCrossAttentionLayer<__nv_bfloat16>;
#endif
